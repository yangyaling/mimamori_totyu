...
+(void)custInfoWithParam:(MCustInfoParam *)param success:(void (^)(NSArray *))success failure:(void (^)(NSError *))failure{
    ...
    [MHttpTool postWithURL:@"http://mimamori.azurewebsites.net/zwgetcustlist.php" params:param.mj_keyValues success:^(id json) {
        if (success) {
...
+(void)custDeleteWithParam:(MCustDeleteParam *)param success:(void (^)(NSString *code))success failure:(void (^)(NSError *error))failure{
    [MHttpTool postWithURL: @"http://mimamori.azurewebsites.net/zwdeletecust.php" params:param.mj_keyValues success:^(id json) {
        if (success) {
...
+(void)custAddWithParam:(MCustAddParam *)param success:(void (^)(NSString *code))success failure:(void (^)(NSError *error))failure{
    [MHttpTool postWithURL: @"http://mimamori.azurewebsites.net/zwaddcust.php" params:param.mj_keyValues success:^(id json) {
        if (success) {
            NSString *code = [json objectForKey:@"code"];
            success(code);
        }
