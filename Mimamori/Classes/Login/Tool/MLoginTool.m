...
+(void)loginWithParam:(MLoginParam *)param success:(void (^)(MLoginResult *result))success failure:(void (^)(NSError *error))failure{
    ...
    [MHttpTool postWithURL:@"http://mimamori.azurewebsites.net/zwlogin.php" params:param.mj_keyValues success:^(id json) {
        if (success) {
            // 字典转模型
...
+(void)sessionInfoWithParam:(MSessionInfoParam *)param success:(void (^)(NSArray *array))success failure:(void (^)(NSError *error))failure{
    ...
    [MHttpTool postWithURL:@"http://mimamori.azurewebsites.net/zwgetzsessioninfo.php"params:param.mj_keyValues success:^(id json) {
        if (success) {
            // 字典数组转模型数组
